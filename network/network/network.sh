...
function createOrgs() {
  ...
  if [ "$CRYPTO" == "Certificate Authorities" ]; then
    ...
    while :; do
      if [ ! -f "organizations/fabric-ca/parkingprovider1.com/tls-cert.pem" ]; then
        sleep 1
      else
        break
      fi
    ...
    infoln "Creating parking provider 1 Identities"
    createParkingProvider1

    infoln "Creating parking provider 2 Identities"
    createParkingProvider2

    infoln "Creating parking integrator Identities"
    createParkingIntegrator
...
CLI_DELAY=3
# channel name defaults to "parking"
